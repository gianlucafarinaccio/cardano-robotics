/* v1.1
   RoboCommunication_v1.c
   Gianluca Farinaccio, 2019
*/

#include <RoboGlobal_v5p.c>
#include <stdlib.h>
...
/* Se come parametro viene passato 1, la richiesta dati al RPi avviene con il
    pin definito nel RoboGlobal.c, attivando in questo modo una routine di
    interrupt sul RPi.
  Se come parametro viene passato 0, la richiesta dati al RPi avviene via RS232
    con l'invio della stringa 'R\r' .                                         */
void send_request(){
    fputc('R',RPi);
    fputc('\r',RPi);
}

/* Decodifica dei dati ricevuti via RS232 dal RPi. */
void str_decode(){
   char data[4];
